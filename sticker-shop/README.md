...
### Fuzzing
Let's see if we can find more resources on the server via `gobuster`. Since no domain name for the website is known, we can only enumerate directories, not subdomains. Additionally, we will also look for files with the extensions `.txt`, `.py`, `.bak` and `.sql`, to find potential backups or source code.
